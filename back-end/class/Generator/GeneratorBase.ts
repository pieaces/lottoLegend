...
export default class GeneratorBase{
    ...
    setInclude(numbers:LottoNumber[]):boolean{
        ...
        if(Calculate.oddCount(numbers) > this.oddCount){
            console.log('!!');
        return false;
        }
        if(Calculate.primeCount(numbers) > this.primeCount){
            console.log('!!!');

            return false;
        }
        if(Calculate.$3Count(numbers) > this.$3Count){
            console.log('!!!!');
            return false;
        }
        if(this.sum$10.to <= Calculate.sum$10(numbers)){
            console.log('!!!!!');
            return false;
        }
        return true;
    }
    setExclude(numbers:LottoNumber[]):boolean{
