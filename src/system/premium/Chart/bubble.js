...
export default class BubbleChart {
    ...
    drawChart() {
        const dataBubble = new google.visualization.DataTable();
        //[['ID', '전체 포화도', '최근 포화도', '종합계수', '확률']];
        dataBubble.addColumn('string', 'ID');
        dataBubble.addColumn('number', '전체 포화도');
        dataBubble.addColumn('number', '최근 포화도');
        dataBubble.addColumn('number', '종합계수');
        dataBubble.addColumn('number', '확률');
        ...
        for (let i = 0; i < labels.length; i++) {
            ...
            X.push(x), Y.push(y);
            _dataBubble.push([labels[i].toString(), x, y, Math.pow(2, x) * Math.pow(2, y), statsData.pos[i]]);
        }
        ...
        dataBubble.addRows(_dataBubble);
        const option = {
